...
class QWenMLP(nn.Module):
    ...
    def forward(self, hidden_states):
        if self.adpt_sign == 0:
            ...
            intermediate_parallel = a1 * F.silu(a2)
            output = self.c_proj(intermediate_parallel)
            adapter_out = torch.matmul(F.silu(torch.matmul(hidden_states[0], self.adpt_w1[0].T)), self.adpt_w2.T)
...
class QWenModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is None and torch.any(input_ids == self.config.visual['image_start_id']):
            ...
            # modification
            self.h[0].mlp.vision_token = images
        ...
        # initialize the vision retracing sign
        layer = 0
        entropy_list = []
        vision_retracing_method = self.h[0].mlp.vision_retracing_method
        vision_token = self.h[0].mlp.vision_token
        retracing_ratio = self.h[0].mlp.retracing_ratio
        entropy_threshold = self.h[0].mlp.entropy_threshold
        starting_layer = self.h[0].mlp.starting_layer
        ending_layer = self.h[0].mlp.ending_layer
        self.h[0].mlp.vision_retracing_event = False
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.h[layer].mlp.vision_retracing_sign:
                ...
                if vision_retracing_method == "adapt":
                    if vision_token is not None:
                        ...
                        self.h[layer].mlp.adpt_w1 = torch.nn.Parameter(torch.zeros_like(vision_token))
                        self.h[layer].mlp.adpt_w2 = torch.nn.Parameter(torch.zeros_like(vision_token.T))
                        print("\n added vision token with adatption channel at layer ", layer)
                    
                self.h[layer].mlp.vision_retracing_sign = False
            ...
            if entropy > entropy_threshold and self.h[0].mlp.vision_retracing_event == False and layer > starting_layer and layer < ending_layer:
                ...
                self.h[0].mlp.vision_retracing_event = True

                if vision_retracing_method == "adapt" :
                    if vision_token is not None:
                        self.h[layer+1].mlp.adpt_sign = 1
                        self.h[layer+1].mlp.adpt_w1 = torch.nn.Parameter(torch.zeros_like(vision_token))
                        self.h[layer+1].mlp.adpt_w2 = torch.nn.Parameter(torch.zeros_like(vision_token.T))
                        self.h[layer+1].mlp.adpt_w1 += (torch.mean(torch.abs(self.h[layer+1].mlp.w1.weight)) / (torch.mean(torch.abs(vision_token))))  * vision_token
                        self.h[layer+1].mlp.adpt_w2 += (torch.mean(torch.abs(self.h[layer+1].mlp.c_proj.weight)) / (torch.mean(torch.abs(vision_token))))  * vision_token.T
