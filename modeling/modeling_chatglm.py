...
class MLP(torch.nn.Module):
    ...
    def forward(self, hidden_states):
        if self.adpt_sign == 0:
            ...
            intermediate_parallel = self.dense_h_to_4h(hidden_states)
            intermediate_parallel = self.activation_func(intermediate_parallel)
            ffn_out = self.dense_4h_to_h(intermediate_parallel)
            adapter_out = torch.matmul(hidden_states, self.adpt_w1.T)
            adapter_out = F.silu(adapter_out)
            adapter_out = torch.matmul(adapter_out, self.adpt_w2.T)
...
class GLMTransformer(torch.nn.Module):
    ...
    def forward(
        ...
        input_ids = self.layers[0].input_ids
        entropy_list = []
        ...
        for index in range(self.num_layers):
            ...
            # nrom_hidden_states = self.final_layernorm(hidden_states)
            ...
            if self.layers[index].mlp.vision_retracing_sign:
                ...
                if vision_retracing_method == "adapt":
                    self.layers[index].mlp.adpt_sign = 0
                    self.layers[index].mlp.adpt_w1 = torch.nn.Parameter(torch.zeros_like(vision_token))
                    self.layers[index].mlp.adpt_w2 = torch.nn.Parameter(torch.zeros_like(vision_token.T))
                    print("\n added vision token with adatption channel at layer ", index)
            ...
            if entropy > entropy_threshold and self.layers[0].mlp.vision_retracing_event == False and index > starting_layer and index < ending_layer:
                ...
                if vision_retracing_method == "adapt":
                    ...
                    self.layers[index+1].mlp.adpt_sign = 1
                    self.layers[index+1].mlp.adpt_w1 = torch.nn.Parameter(torch.zeros_like(vision_token))
                    self.layers[index+1].mlp.adpt_w2 = torch.nn.Parameter(torch.zeros_like(vision_token.T))
                    self.layers[index+1].mlp.adpt_w1 += (torch.mean(torch.abs(self.layers[index+1].mlp.dense_h_to_4h.weight)) / (torch.mean(torch.abs(vision_token))))  * vision_token
                    self.layers[index+1].mlp.adpt_w2 += (torch.mean(torch.abs(self.layers[index+1].mlp.dense_h_to_4h.weight)) / (torch.mean(torch.abs(vision_token))))  * vision_token.T
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        self.encoder.layers[0].input_ids = input_ids
        ...
        if past_key_values is None:
            ...
            if not is_empty(images):  # multi-modality
                ...
                self.encoder.layers[0].mlp.vision_token = images_features[0]
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    def __init__(self, config: ChatGLMConfig, empty_init=True, device=None):
        ...
        self.config = config
        self.transformer.encoder.layers[0].mlp.vision_retracing_method = "default"
        self.transformer.encoder.layers[0].mlp.vision_token = None
        self.transformer.encoder.layers[0].mlp.retracing_ratio = 0
        self.transformer.encoder.layers[0].mlp.entropy_threshold = 1
        self.transformer.encoder.layers[0].mlp.starting_layer = 0
        self.transformer.encoder.layers[0].mlp.ending_layer = 0
        self.transformer.encoder.layers[0].mlp.vision_retracing_event = False
    ...
    def forward(
        ...
        self.transformer.encoder.output_layer = self.transformer.output_layer
